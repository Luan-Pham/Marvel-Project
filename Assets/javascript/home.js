...
function search() {
  ...
  fetch(characterURL)
    ...
    .then(function (data) {
      ...
      var movieURL =
        "https://api.themoviedb.org/3/search/movie?api_key=287dc70aa17f7ea83559b0c661966d3e&language=en-US&page=1&include_adult=false&query=" +
        characterName;
      fetch(movieURL)
        .then((response) => response.json())
        .then(function (data) {
          ...
          if (data.results.length == 0) {
            $("#movies").attr(
              "src",
              "http://x.annihil.us/u/prod/marvel/i/mg/3/40/4bb4680432f73/portrait_uncanny.jpg"
            );
          } else {
            $("#movies").attr(
              "src",
